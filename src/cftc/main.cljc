(ns cftc.main
  "Kotodama WASM entrypoint for the Cftc clean-room actor (L4) — Clojure port.
  ...
  facts (`cftc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cftc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cftc-compat" :tier tier :entities entities} 200])
